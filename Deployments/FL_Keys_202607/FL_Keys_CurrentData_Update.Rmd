...
cat("SARGE DATE UPDATE: FLORIDA KEYS DEPLOYMENT: July 27th to 28th, 2026")
...
## Plot 
waterlev <- ggplot(exo, aes(x=TIMESTAMP, y=Depth_m)) + geom_point(color="blue") + ylim(-0.5,2.5) + xlim(c(as.POSIXct('2026-07-27 12:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + 
   theme_classic() + labs(title="EXO Water Level")
waterlev

## Plot conductivity 
cond <- ggplot(exo, aes(x=TIMESTAMP, y=Salinity_PPT)) + geom_point(color="purple") + 
xlim(c(as.POSIXct('2026-07-27 12:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + ylim(20,50) + 
  theme_classic() + labs(title="EXO Salinity - CURRENTLY NOT WORKING!")
cond
...
sontek$Water_temperature <- as.numeric(sontek$Water_temperature)
#head(sontek)

## Plot LGR Data for quick look
flow <- ggplot(sontek, aes(x=TIMESTAMP, y=Flowrate)) + geom_point(color="black") +       
xlim(c(as.POSIXct('2026-07-27 12:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + theme_classic() + 
      labs(title="Sontek Flow Rate")
#flow

flow1 <- ggplot(sontek, aes(x=TIMESTAMP, y=as.numeric(Index_velocity))) + geom_point(color="black") +       
xlim(c(as.POSIXct('2026-07-27 18:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + ylim(-0.4, 0.3) + 
...
xlim(c(as.POSIXct('2026-07-27 18:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + theme_classic() + ylim(0.1,1.5) + 
      labs(title="Sontek Water Depth")
sdepth

stemp <- ggplot(sontek, aes(x=TIMESTAMP, y=Water_temperature)) + geom_point(color="darkred") +
xlim(c(as.POSIXct('2026-07-27 18:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + theme_classic() + #ylim(0.1,1.5) + 
      labs(title="Sontek Water Temperature")
stemp
```
...
CH4_lgr <- ggplot(flux, aes(x=TIMESTAMP1, y=CH4d_ppmA)) + geom_point(color="magenta")+
  xlim(c(as.POSIXct('2026-07-27 16:00:00', format = "%Y-%m-%d %H:%M:%S"),
     as.POSIXct('2026-07-28 12:00:00', format = "%Y-%m-%d %H:%M:%S"))) + ylim(0,7000)+
