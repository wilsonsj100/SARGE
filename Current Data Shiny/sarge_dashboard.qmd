...
output$plot_hydrology <- renderPlotly({
  p1 <- plot_sarge(hydrology, 
                   input$start_date, 
                   input$end_date, 
                   input$hydrology_vars,
                   input$hydrology_colors)
...
output$data_latest <- renderText({
  ...
  formatted_date_exo <- paste(
    format(
      max(exo$TIMESTAMP, na.rm = T),
      "%Y-%m-%d %H:%M:%S"
    ),
    TIMEZONE
  )
  ...
  formatted_date_rad <- paste(
    format(
      max(radon$TIMESTAMP, na.rm = T),
      "%Y-%m-%d %H:%M:%S"
    ),
    TIMEZONE
  )
  ...
  formatted_date_hydrology <- paste(
    format(
      max(hydrology$TIMESTAMP, na.rm = T),
      "%Y-%m-%d %H:%M:%S"
    ),
    TIMEZONE
  )
